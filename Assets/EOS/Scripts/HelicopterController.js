...
class HelicopterController extends VehicleController {
	...
	private var rotorAxis : Vector3;
	var responsiveness : float = 0.1;
	...
	function UpdateRotorGraphics(relativeVelocity : Vector3)
	{
		...
		if (rotor) {
			var formerAxis = Vector3.Cross(rotor.localRotation * Vector3.up, Vector3.up);
			var formerAngle = Mathf.Asin(formerAxis.magnitude) * Mathf.Rad2Deg;	
					
			var tmpRot = rotor.localRotation * Quaternion.AngleAxis(1000 * Time.deltaTime, Vector3.up);
			tmpRot = Quaternion.AngleAxis(angle, axis) * Quaternion.AngleAxis(formerAngle, formerAxis.normalized) * tmpRot;
			rotor.localRotation = tmpRot;		
		}
	
		body.localRotation = Quaternion.AngleAxis(angle, axis);
	}
	...
	private var forwardForce:Vector3;
	private var upForce:Vector3;
