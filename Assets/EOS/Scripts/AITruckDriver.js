...
class AITruckDriver extends AIDriver {
	...
	function Drive(goal:Vector3) {
		...
		var control1 = Mathf.Max(0, Mathf.Min(maxThrottle, p1 * ref1 + d1 * (ref1 - history1)));
		var control2 = Mathf.Max(-1, Mathf.Min(1, p2 * ref2 + d2 * (ref2 - history2)));
		
		if (Mathf.Abs(control2) > 0.1 && control1 < maxThrottle) {
			control1 = maxThrottle;
		}
